...
export async function PUT(request:NextRequest,{ params }: { params: { id: string } }){
    try{
        await connect();
        const {id}=params;
        const {name,category,imageUrl,ingredients,instructions,isFavorite}=await request.json();
        ...
        updateRecipe.isFavorite=isFavorite||updateRecipe.isFavorite;
        await updateRecipe.save();
        return NextResponse.json({message:"Successfully updated",updateRecipe:updateRecipe,status:200})
    }catch(error){
